...
SELECT 
    ...
    CASE 
        ...
        ELSE 'IL' -- Default to Israel as most data seems to be from there
    END as Country,
    NULL as PostalCode,
...
-- Import Supplier Companies
INSERT INTO Companies (Id, Name, CompanyType, Website, Email, Phone, Address, City, Country, PostalCode, Description, IsActive, CreatedAt, UpdatedAt)
SELECT TOP 500 -- Start with top 500 suppliers
    NEWID() as Id,
    ...
    END as Country,
    PostalCode,
    CONCAT('Category: ', ProductCategory, ' | ', LEFT(Description, 200)) as Description,
    1 as IsActive,
    GETUTCDATE() as CreatedAt,
    GETUTCDATE() as UpdatedAt
FROM FoodXSuppliers
...
SELECT 
    NEWID() as Id,
    -- Link to test buyer users in round-robin fashion
    CASE (ROW_NUMBER() OVER (ORDER BY c.Name) % 3)
        WHEN 1 THEN (SELECT Id FROM AspNetUsers WHERE Email = 'buyer1@test.fdx.trading')
        WHEN 2 THEN (SELECT Id FROM AspNetUsers WHERE Email = 'buyer2@test.fdx.trading')
        ELSE (SELECT Id FROM AspNetUsers WHERE Email = 'buyer3@test.fdx.trading')
    END as UserId,
    c.Id as CompanyId,
...
SELECT 
    NEWID() as Id,
    -- Link to test supplier users in round-robin fashion
    CASE (ROW_NUMBER() OVER (ORDER BY c.Name) % 3)
        WHEN 1 THEN (SELECT Id FROM AspNetUsers WHERE Email = 'supplier1@test.fdx.trading')
        WHEN 2 THEN (SELECT Id FROM AspNetUsers WHERE Email = 'supplier2@test.fdx.trading')
        ELSE (SELECT Id FROM AspNetUsers WHERE Email = 'supplier3@test.fdx.trading')
    END as UserId,
    c.Id as CompanyId,
    'Manufacturer' as SupplierType,
    SUBSTRING(c.Description, CHARINDEX('Category: ', c.Description) + 10, 50) as ProductCategories,
...
SELECT TOP 1000
    ...
    CASE (ABS(CHECKSUM(NEWID())) % 8)
        ...
        ELSE 'Beverages'
    END as Category,
    CONCAT('SKU-', LEFT(NEWID(), 8)) as SKU,
...
PRINT '';
PRINT '=== TOP 10 SUPPLIER COMPANIES ===';
SELECT TOP 10 
    c.Name,
    c.Country,
